...
def get_exchange_updates():
    ...
    請按以下順序與格式輸出 HTML 內容（不要包含 ```html 標籤字眼）：

    1. **摘要區塊**：
       在最上方寫出「本週競品產品亮點功能更新摘要」，並建議最適合我們中小型交易所跟進的功能。
       要求：
       - 必須使用 HTML 的 <ul> 和 <li> 標籤做分段重點。
       - 總字數嚴格控制在 150 字以內。
       - 格式範例：<h3>📌 本週動態摘要</h3><ul><li>重點一...</li><li>建議跟進...</li></ul>

    2. **表格區塊**：
       接續輸出 HTML 表格，表格標籤包含 border="1" 和 cellpadding="5"。
       欄位：交易所名稱、功能亮點、影響程度(High/Medium/Low)、來源(文字名稱)。

    要求：
    - 僅限產品功能、App改版、交易工具等更新。
    - 使用繁體中文。
    - 表格中不要放連結。
    """
